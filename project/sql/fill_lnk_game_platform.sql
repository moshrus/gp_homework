create or replace function project.fill_lnk_game_platform ()
returns void
language plpgsql
as $body$
declare
  lc_TableName      constant varchar := 'project.lnk_game_platform';
...
begin
  ...
  insert into project.lnk_game_platform (
  ...
  with wt_data as (
    select distinct md5(md5(p.game_id::text || md5(p.platform::text))) as lnk_game_platform_key
         ...
         , md5(p.platform::text) as platform_key
  ...
  select d.lnk_game_platform_key
    ...
    from wt_data d
    left
    join project.lnk_game_platform l
      on d.lnk_game_platform_key = l.lnk_game_platform_key
   where l.lnk_game_platform_key is null
    ;
...
comment on function project.fill_lnk_game_platform() is 'Функция загрузки данных в таблицу project.lnk_game_platform';
